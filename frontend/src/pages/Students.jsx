    ...
    <Box sx={{ p: 0 }}>
                  ...
                  <TableRow key={student.id} hover sx={{ '&:last-child td': { border: 0 } }}>
                    ...
                    <TableCell>
                      <Box sx={{ display: 'flex', alignItems: 'center', gap: 1.5 }}>
                        <Avatar src={student.photo ? `${import.meta.env.VITE_API_URL || 'http://localhost:3999'}/file/${student.photo}` : undefined} sx={{ width: 32, height: 32, backgroundColor: getColor(student.id), fontSize: '0.75rem', fontWeight: 700 }}>{getInitials(student.full_name)}</Avatar>
                        <Typography sx={{ fontWeight: 600, fontSize: '0.85rem' }}>{student.full_name}</Typography>
                      </Box>
                    </TableCell>
                    <TableCell>
                      <Box sx={{ display: 'flex', gap: 0.5, flexWrap: 'wrap' }}>
                        {groups.length > 0 ? groups.map((g, i) => <Chip key={i} label={g} size="small" sx={{ fontSize: '0.7rem', height: 22, backgroundColor: '#faede0', color: '#c5a059', fontWeight: 600 }} />) : <Typography sx={{ fontSize: '0.8rem', color: '#9ca3af' }}>—</Typography>}
                      </Box>
                    </TableCell>
                    ...
                    <TableCell>
                      {activeTab === 'archive' ? (
                        ...
                        <Stack direction="row" spacing={0.5}>
                          <IconButton size="small" sx={{ '&:hover': { color: '#c5a059' } }}><VisibilityIcon sx={{ fontSize: 18 }} /></IconButton>
        ...
        <Box sx={{ p: 2, display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderTop: '1px solid #e5e7eb' }}>
          ...
          <Box sx={{ display: 'flex', gap: 0.5 }}>
            {Array.from({ length: Math.min(totalPages, 5) }, (_, i) => i + 1).map(p => (
              <Button key={p} size="small" onClick={() => setPage(p)} sx={{ minWidth: 32, height: 32, borderRadius: '8px', fontWeight: page === p ? 700 : 400, backgroundColor: page === p ? '#c5a059' : 'transparent', color: page === p ? '#fff' : '#4b5563', '&:hover': { backgroundColor: page === p ? '#b89350' : '#f3f4f6' } }}>{p}</Button>
            ))}
          </Box>
      ...
      <Drawer
        ...
        <Box sx={{ p: 3, overflowY: 'auto', flex: 1 }}>
          <Stack spacing={2.5}>
            <Box>
              ...
              <TextField fullWidth size="small" placeholder="+998" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} autoComplete="off" sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px' } }} />
            </Box>

            <Box>
              <Typography sx={{ mb: 0.5, fontWeight: 600, fontSize: '0.82rem', color: '#374151' }}>Mail</Typography>
              <TextField fullWidth size="small" placeholder="Elektron pochtani kiriting" value={form.email} onChange={(e) => setForm({ ...form, email: e.target.value })} autoComplete="off" sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px' } }} />
            </Box>

            <Box>
              <Typography sx={{ mb: 0.5, fontWeight: 600, fontSize: '0.82rem', color: '#374151' }}>Talaba FIO</Typography>
              <TextField fullWidth size="small" placeholder="Ma'lumotni kiriting" value={form.full_name} onChange={(e) => setForm({ ...form, full_name: e.target.value })} autoComplete="off" sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px' } }} />
            </Box>

            <Box>
              <Typography sx={{ mb: 0.5, fontWeight: 600, fontSize: '0.82rem', color: '#374151' }}>Tug'ilgan sanasi</Typography>
              <TextField fullWidth size="small" type="date" value={form.birth_date} onChange={(e) => setForm({ ...form, birth_date: e.target.value })} autoComplete="off" sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px' } }} />
            </Box>
            ...
            <Box>
              ...
              <Box component="label" sx={{ border: '1.5px dashed #e5e7eb', borderRadius: '10px', p: 3, textAlign: 'center', cursor: 'pointer', display: 'block', '&:hover': { borderColor: '#c5a059' } }}>
                ...
                <Typography sx={{ fontSize: '0.75rem', color: '#c5a059', fontWeight: 600 }}>{form.photo ? form.photo.name : 'Click to upload or drag and drop'}</Typography>
            ...
            <Box>
              ...
              <TextField fullWidth size="small" placeholder="Manzilni kiriting" value={form.address} onChange={(e) => setForm({ ...form, address: e.target.value })} autoComplete="off" sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px' } }} InputProps={{ startAdornment: <InputAdornment position="start"><PlaceIcon sx={{ fontSize: 18, color: '#9ca3af' }} /></InputAdornment> }} />
            </Box>

            <Box>
              <Typography sx={{ mb: 0.5, fontWeight: 600, fontSize: '0.82rem', color: '#374151' }}>Parol</Typography>
              <TextField fullWidth size="small" type="password" placeholder="Parolni kiriting" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} autoComplete="new-password" sx={{ '& .MuiOutlinedInput-root': { borderRadius: '8px' } }} InputProps={{ startAdornment: <InputAdornment position="start"><KeyIcon sx={{ fontSize: 18, color: '#9ca3af' }} /></InputAdornment> }} />
            </Box>
          </Stack>
        </Box>
        ...
        <Box sx={{ p: 2, display: 'flex', gap: 2 }}>
          ...
          <Button fullWidth variant="contained" onClick={handleSubmit} sx={{ backgroundColor: '#c5a059', borderRadius: '8px', textTransform: 'none', '&:hover': { backgroundColor: '#b89350' } }}>Saqlash</Button>
        </Box>
      </Drawer>
