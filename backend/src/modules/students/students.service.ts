...
export class StudentsService {
  ...
  async create(payload: CreateStudentDto, filename: string) {
    ...
    this.emailService
      .sendEmail(payload.email, payload.phone, payload.password)
  ...
  async update(id: number, payload: UpdateStudentDto, filename?: string) {
    ...
    const isPhoneChanged = !!(
      payload.phone &&
      normalizePhone(payload.phone) !== normalizePhone(student.phone)
    );
    ...
    if (isPasswordChanged || isPhoneChanged) {
      ...
      this.smsService
        ...
        .catch((err) =>
          console.error("Student update SMS xatolik:", err.message),
        );
    }
