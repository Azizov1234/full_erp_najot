...
export class HomeWorksService {
    ...
    if (currentUser.role === UserRole.TEACHER) {
      ...
      if (answer.homeworkStatus === HomeworkStatus.ACCEPTED) {
        throw new ForbiddenException(
          "Siz bu vazifani qaytadan baholay olmaysiz",
        );
      }
    }
  ...
  async submitHomework(
    ...
    if (existing) {
      if (
        existing.homeworkStatus === HomeworkStatus.RETURNED &&
        !existing.allow_resubmit
      ) {
        throw new ForbiddenException(
          "Sizga qayta topshirish uchun ruxsat berilmagan",
        );
      }
      // Update existing
